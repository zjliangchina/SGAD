...
def Indd_train(clf, train_data, test_data, OneEncoded):
    ...
    np.savetxt("./Datasets/" + disease + "/SGAD.txt", save_figdata, fmt='%0.6f')
...
class ModelCon(nn.Module):
    def __init__(self, node_size, nhid0, nhid1, droput, alpha):
        ...
        self.c = Nodecount
        self.clf_layer = nn.Sequential(nn.Linear(1218 * 2 + 4 * 32 + 3, self.dense_dim1),
...
if __name__ == '__main__':
    ...
    proteinAC = CodingAC(protein)
    proteinLD = CodingLD(protein)
    proteinCT = CodingCT(protein)
    proteinPseAAC = CodingPseAAC(protein)
