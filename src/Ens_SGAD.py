...
def Indd_stacking2(clf, train_data, test_data, OneEncoded, n_folds=5):
    train_num, test_num = len(train_data), len(test_data)  # The number of training sets is 6204, the number of test sets is 1552
    second_level_train_set = np.zeros((train_num,))  # np.zeros((n,)) is an n-dimensional array, in the form of 1*n, corresponding to the second-level training set
    second_level_test_set = np.zeros((test_num,))  # np.zeros((n,)) is an n-dimensional array in the form of 1*n, corresponding to the second layer test set
    ...
    for i, (train_index, test_index) in enumerate(kf.split(train_data_)):  # Divide the 6204 pieces of data in the training set into 5 parts
        print("n_folds = %d" % i)
...
def text_save(filename, data):  # filename is the path to write the CSV file, data is the list of data to be written.
    ...
    for i in range(len(data)):
        s = str(data[i]).replace('[', '').replace(']', '')  # Remove [], the two lines are different according to the data, you can choose
...
if __name__ == '__main__':
    ...
    num_model = 0
    for i in range(len(proteindict)):  # There are three different encoding forms
        if i == 0:
    ...
    criterion = nn.CrossEntropyLoss()  # Define the loss function as cross entropy
    optimizer = torch.optim.SGD(netClaDNN.parameters(), 1e-1)  # Optimal use of stochastic gradient descent, learning rate 0.1
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=args.step_size, gamma=args.gamma)
    ...
    np.savetxt("./Datasets/" + disease + "/Ens_SGAD.txt", save_figdata, fmt='%0.6f')
